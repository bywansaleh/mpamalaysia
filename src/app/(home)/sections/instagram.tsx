...
export default async function Instagram() {
  ...
  return (
    <div className={`
      relative z-100 bg-black bg-linear-to-br from-brand to-brand/50
      shadow-[0_0_100px_hsl(var(--brand)/40%)]
    `}
    >
  ...
  return (
    <a
      ...
      className={cn(
        // styles.card,
        `
          group relative flex flex-col gap-4 overflow-hidden rounded-md bg-black
    ...
    >
      <img
        ...
        className={`
          ...
          ease-in-out
          [mask:linear-gradient(to_bottom,black,rgba(0,0,0,0.3)_80%)]
          group-hover:scale-105 group-hover:opacity-60 group-hover:blur-sm
        `}
      />
      <div className="absolute inset-0 flex flex-col justify-end gap-2 p-2">
        <p className={`
          line-clamp-8 text-2xs leading-tight! font-medium text-pretty opacity-0
        ...
        <div className={`
          flex w-full flex-row-reverse items-center justify-between gap-1
          text-2xs font-semibold tracking-tighter
        `}
        >
          <svg
